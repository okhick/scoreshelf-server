...
export class AssetProcessing {
  ...
  async makePdfThumbnail(pdf: Buffer, page: number): Promise<path.ParsedPath> {
    ...
    if (!existsSync(this.PDF2PIC_TEMP_SAVE_PATH)) { mkdirSync(this.PDF2PIC_TEMP_SAVE_PATH) }
  ...
  getPdfDimensions(pdfJSON: PDFJson, pageIndex: number): 
  ...
  setImageDimensions(ratio: number): 
